...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        self.convolutional_layers = [Conv1d(num_input_channels, num_channels[0], kernel_sizes[0], strides[0], conv_weight_init_fn, bias_init_fn),
                                     Conv1d(num_channels[0], num_channels[1], kernel_sizes[1], strides[1], conv_weight_init_fn, bias_init_fn),
                                     Conv1d(num_channels[1], num_channels[2], kernel_sizes[2], strides[2], conv_weight_init_fn, bias_init_fn)]
        self.flatten = Flatten()
        self.linear_layer = Linear(num_channels[2] * (input_width // (strides[0] * strides[1] * strides[2])), num_linear_neurons, linear_weight_init_fn, bias_init_fn)
        self.layers = self.convolutional_layers + [self.flatten, self.linear_layer]
